...
RSpec.describe AcceptLanguage::Matcher do
  ...
  describe "Regex injection protection" do
    context "when preferred tag contains dot pattern" do
      let(:matcher) { described_class.new("en.US" => BigDecimal("1.0")) }

      it "does not match unintended tags" do
        expect(matcher.call("en-US")).to be_nil
      end

      it "only matches literal dot" do
        expect(matcher.call("en.US")).to eq "en.US"
      end
    end

    context "when preferred tag contains dot star pattern" do
      let(:matcher) { described_class.new(".*" => BigDecimal("1.0")) }

      it "does not match everything" do
        expect(matcher.call("en", "fr", "de")).to be_nil
      end
    end

    context "when preferred tag contains alternation" do
      let(:matcher) { described_class.new("en|fr" => BigDecimal("1.0")) }

      it "does not match en" do
        expect(matcher.call("en")).to be_nil
      end

      it "does not match fr" do
        expect(matcher.call("fr")).to be_nil
      end
    end

    context "when preferred tag contains character class" do
      let(:matcher) { described_class.new("[a-z]" => BigDecimal("1.0")) }

      it "does not match single letters" do
        expect(matcher.call("a", "b", "c")).to be_nil
      end
    end

    context "when excluded tag contains regex pattern" do
      let(:matcher) { described_class.new("*" => BigDecimal("1.0"), "e." => BigDecimal("0")) }

      it "does not exclude unintended tags" do
        expect(matcher.call("en")).to eq "en"
      end
    end
  end
end
